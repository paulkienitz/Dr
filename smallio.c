...
We make the argument to puch an unsigned short rather than a char just to save
trouble.  The argument to OpenSmallIO is a function to be called to check for a
control-C break and handle it if present.  NULL is okay.  It may call
StopAllOutput().  After StopAllOutput(), further puch/put/putfmt calls will have
no effect.  It does not exist if BACKGROUND is not defined.
