...
>>> The grand ambition: find old source versions (may be difficult), and merge
old fastscan.library-based code with new features to create a 2.1 update which
can run under AmigaDOS 1.2! ....But unfortunately, the source for versions 1.5
and 1.6 seems to be completely lost -- I can't find a copy of the archives I
sent out for distribution, though I thought I'd saved them.  Even 1.3 is lost,
which I think predates fastscan.library... I do have the  final source of
fastscan.library itself (without the makefile entries for it, though).
~~~~~~~~ So let's do as we did with Move 1.2 and inventory the 2.x API calls:
        OpenLocale and GetLocaleStr (v38 but properly conditioned, no problem)
        CompareDates (v36)
        PrintFault (v36)
        ParsePatternNoCase and MatchPatternNoCase (v37)
        ReadLink (v36, somewhat protected in some cases, but maybe not safe)
        System (v36)
        FilePart and PathPart (v36)
        MatchFirst, MatchNext, MatchEnd, and struct AnchorPath (v36)
        LockDosList, FindDosEntry, and UnLockDosList (v36)
        GetDeviceProc (v36)
        GetVar (v36)
I was thinking this was less bad than Move despite its much larger size...
but what does SmallIO call?  Just one thing:
        CreateNewProc (v36)
####### Oh yeah, besides this and Move, let's make CLImax 1.x compatible again.

Substitute #? when splitting out a pathname component yields a bare *.
Maybe add path/::pattern to mean list files that don't match?  No, 3.1 uses ~ for negating?

AAAAARRRRRGGGHH!!!  -I option fucks up alphabetical sorting!

Make command execution lag behind ExNext by one file, so rename/delete is safer!
And execute commands with ORIGINAL current dir so colonless \p works right!

It acts weird when you give it "dir/#?/".

"Couldn't find askdjhfkas" error message should clarify.

*** Test -J more when spanning volumes.

*** TEST NEW SECONDARY ERROR MESSAGE BEHAVIOR.

*** Test fail level thing with letters after, and in DR-OPTS.

Test new -P..|..|.. thing a bit more?  I think it's working okay.

option letters available: E Q W  ...maybe Q = Quick supercompact columns
...
Deal with problem of [\k] and -K producing "[   23]" instead of "   [23]"?
...
the files Dr.doc for useful information.  Documentation for smallio.c is in the
source, kind of.
========================================================================== */
...
/* WIDEFAULT is the assumed output width when we can't measure the window.
   ...
   MAXWEEKLEN is the top length + 1 of internationalized weekday names.
   MAXARGS is the maximum allowable number of command line arguments.
   MAXPROTORS is how many different protection bit sets can be or'd together.
   MULTILIMIT is the number of locks that can be handled in a multiple assign.
*/

#include "env:DrVersion"
...
typedef struct _fly {
    ...
    ubyte /* bool */ jected, infoed, wanted, dirred, softlink;
    ushort fflags;      /* ... and these are the less used ones */
    str slinky;
} fly;                  /* size exactly 80 bytes for historical reasons */
...
short cwid, song, keygits, cols, wid, abort, rdepth, coughed;
short prot_ors = 0, steil = 0;

bool color, curse, cize, complete, cron, cons, cutdirs, ceys, cutfils, colorful;
bool cage, canydepth, csternal, cortless, consumption, ctifle, cweeek, climb_in;
...
bool patty, flipat, didaninny, needsnl, anyfiles, morethan1, specialgap;
bool notadir, notadirbutokay, froggy, overlong = false, goodcough = false;

long before, after, tooday, failat = 10;
...
struct ConUnit *cuca;
struct Process *me;
long hair;            /* dos error code serious enough to not return success */

ubyte mesh[PATLIMIT * 2 + 3];
char abspath[ABSIZE];

ubyte failevels[MAXARGS];
long protlook[MAXPROTORS], protwant[MAXPROTORS];
...
"    -F show only files, not dirs        -G show datestamp as age; days H:M:S\n"
"    -H sort in rows, not columns        -I show .info files normally\n"
"    -J treat soft links as real subdirs -K show disk addresses\n"
...
"    -X show dir's info, not contents    -Y use day names for recent dates\n"
"    -Z sort from smallest to largest    -@ don't use DR-OPTS environment var\n"
"    -? no output; return 5 if no match  -! turn off cursor for faster output\n"
...
"    -Pb where b is any of H S P A R W E D with optional ~ in front and | in\n"
"         between (e.g. -PS~W|~AD): don't show non-matching protection bits\n"
...
str defweekdaynames[7] = { "Sunday", "Monday", "Tuesday", "Wednesday",
                                "Thursday", "Friday", "Saturday" };
char weekdaynames[7][MAXWEEKLEN];
short weeknamelen = 9;
...
{
    pad(w - digits(n));              /* RawDoFmt doesn't have %*lu */
    putfmt("%lu", n);
}
...
_mob:   move.b  d0,(a3)+
        rts
#endasm
void mob(char d, str s);

void ultoa(str dest, ulong val)
{
    RawDoFmt("%lu", &val, mob, dest);
}
...
void formdate(str s, struct DateStamp *when, short space)
{
    ...
    if (cage) {
        if (CompareDates(when, &now) < 0)
            d1 = &now, d2 = when;
        else
            d2 = &now, d1 = when;
        d.sdif = (d2->ds_Tick - d1->ds_Tick) / TICKS_PER_SECOND;
        d.mdif = d2->ds_Minute - d1->ds_Minute;
        d.ddif = d2->ds_Days - d1->ds_Days;
        if (d.sdif < 0)
            d.sdif += 60, d.mdif--;
        if (d.mdif < 0)
            d.mdif += 1440, d.ddif--;
        ...
        d.mdif %= 60;
        temp[0] = '-';
        RawDoFmt("%ldd %02d:%02d:%02d", (adr) &d, mob, temp + (d2 == when));
    } else {
        dt.dat_Stamp = *when;
        ...
        dt.dat_StrDay = null;
        dt.dat_StrDate = temp;
        dt.dat_StrTime = temp + 25;
        DateToStr(&dt);
        l = min(strlen(temp), 9 + froggy);   /* Procrustean i18n */
        temp[l++] = ' ';
        strcpy(temp + l, temp + 25);
    }
    l = strlen(temp);
    while (space > l)
        *s++ = ' ', space--;
...
void CCch(void)
{
    if (abort < 5 && (SetSignal(0L, SIGBREAKF_CTRL_C) & SIGBREAKF_CTRL_C)) {
        abort = 5;
        StopAllOutput();
        Write(Output(), "\n *** BREAK\n", 12L);
        me->pr_Result2 = 0;
...
bool FitzBitz(ulong prot)
{
    register short i;
    if (!prot_ors)
        return true;
    for (i = 0; i < prot_ors; i++)
        if ((prot & protlook[i]) == protwant[i])
            return true;
    return false;
}
...
/* I know, there's an escape sequence.  But it don't work without you sends
a packet to set raw mode... */
...
/*  if (!zons) {
    ...
    if (zomplete) {
        ...
        formdate(buf2, &y->when, 18 + (froggy & cage));
#ifdef MARKLINKS
...
void Header(short ficou, str name)
{
    ...
    if (abort)
        return;
    if ((didaninny | specialgap) && (!consumption || oform || cortless))
        puch('\n');
    if (ficou)
        putfmt("        ===  %s  ===\n", nn);
    else
...
{
    flip filist, f;
    bool bigger, heads, viz = !hasparent;
    ...
    for (pa = ants; pa; pa = pa->prav)
        if (pa->kay == parant.kay) {
            putfmt("\n *** %s is its own ancestor, through a link!\n", prepath);
            specialgap = true;
            /* don't set hair */
            goto skipit;
        }
...
bool SplitTailPat(str from, BPTR *deer)
{
    ...
    short w;
    bool give_up = me->pr_Result2 == ERROR_DEVICE_NOT_MOUNTED
                        || me->pr_Result2 == ERROR_NO_DISK;
                ...
                if (lox[l]) {
                    if (!abort)
                        if (NameFromLock(lox[l], abspath, ABSIZE)) {
                            DoInner(abspath, lox[l], false, null, null);
                        } else {
                            h = me->pr_Result2;
                            didaninny = true;
                            pflush();
...
void Do(str what)
{
    ...
    if (deer == -1)
        deer = RLock(what);
    if (!deer && !SplitTailPat(what, &deer))
        return;
    if (ankh) {
        morethan1 = true;
        do {
            if (!patty || ankh->ap_Info.fib_EntryType > 0) {
                ...
                if (deer = RLock(ankh->ap_Info.fib_FileName)) {
                    *prepath = 0;
                    ...
                    DoInner(abspath, deer, false, null, null);
                    UnLock(deer);
                } else {
                    hair = me->pr_Result2;
                    putfmt("\n *** Can't lock directory \"%s\"!\n", abspath);
                }
                CurrentDir(ocd);
            }
            CCch();
        ...
        if (convertpath && NameFromLock(deer, abspath, ABSIZE))
            what = abspath;
        *prepath = 0;
        DoInner(what, deer, false, null, null);
        UnLock(deer);
    }
}
...
void Opt(register ustr p, ubyte fai)
{
    register ubyte c;
            ...
            case 'p':  {
                ...
                while ((c = lower(p[1])) > ' ') {
                    ...
                    switch (c) {
                        ...
                        case '|':
                            prot_ors++;
                            if (prot_ors < MAXPROTORS)
                                protlook[prot_ors] = protwant[prot_ors] = 0;
                            else
                                putn(
" *** more than seven \"|\" separators after -P; further bits ignored.");
                            continue;           /* no value for b needed */
                        default:  b = -1;
                    }
                    if (b < 0)
                        putn(
" *** letters after -P must be any of H S P A R W E D.");
                    else if (prot_ors < MAXPROTORS) {
                        if (prot_ors < 0)
                            prot_ors = 0;
                        protlook[prot_ors] |= bit(b);
                        if (tilt ^ (b >= 4))
                            protwant[prot_ors] |= bit(b);
                    }
                }
                if (prot_ors < MAXPROTORS)
                    prot_ors++;
...
void mane(void)
{
    ...
    if (LocaleBase = OpenL("locale")) {         /* failure is OK */
        ll = OpenLocale(null);
        froggy = !strcmp(ll->loc_LanguageName, "français.language");
        weeknamelen = 0;
        for (a = 0; a < 7; a++) {
            if (p = GetLocaleStr(ll, DAY_1 + a))
                defweekdaynames[a] = p;
            if ((aa = strlen(defweekdaynames[a])) > weeknamelen)
                weeknamelen = aa;
        }
        if (weeknamelen >= MAXWEEKLEN)
            weeknamelen = MAXWEEKLEN - 1;
    }
    for (a = 0; a < 7; a++) {           /* pad names to a constant length */
        p = defweekdaynames[a];
        strncpy(weekdaynames[a], p, weeknamelen);
        for (aa = strlen(p); aa < weeknamelen; aa++)
            weekdaynames[a][aa] = ' ';
        weekdaynames[a][weeknamelen] = 0;
    }
    if (help) {         /* -cdfhiklorsux | -a# | -b# | -p... | -{...} | -[...] */
        putfmt("Dr " VERSION " by Paul Kienitz"
    ...
    if (needsnl)
        puch('\n');
    if (goodcough && hair && !ctifle && hair != ERROR_OBJECT_NOT_FOUND
    ...
    *tempargv = argline;
    ...
    if (alen > 0) {
        ...
        for (;;) {
            ...
            flevel = 10;
            if (quoted && *ap == unquote) {
                ap++;
                if (unquote == '}')
                    if (isdigit(*ap)) {
                        flevel = 0;
                        while (isdigit(*ap))
                            flevel = 10 * flevel + *ap++ - '0';
                    }
            }
            *(poik++) = '\0';
            failevels[argc] = flevel;
            tempargv[++argc] = poik;
